...
class ApiGateway2ApplicationTests {
	...
	@Test
	void contextLoads() {
		System.out.println("테스트가 성공적으로");
	}
	/*
	 * 1. api 연동을 위한 기반데이터준비
	 * 2. 데이터를 xml로 변환할 기능추가
	 * 3. rest api로 전달하기 위한 resttemplate를 준비한다
	 * 4. api전송시 필요한 header값을 설정한다.
	 * 5. 준비한 데이터의 인코딩을 조정한다
	 * 6. 준비된 데이터를 기반으로 url을 생성
	 * 7.requestㅇ청시 저ㄴ송한 entitiy를 구성한다.
	 * 8. rest api를 전송한다
	 * 9. 응답받은 response를 응답한다
	 */
	...
	void test() throws Exception{
		...
		// 2. 데이터를 XML로 변환할 기능을 추가한다.
				...
				List<HttpMessageConverter<?>> converters = new ArrayList<HttpMessageConverters<?>>();
